...
AgeLimitDays = 30
AgeLimit = AgeLimitDays*60*60*24   #30 day age limit for ODB files
...
timed_ext = [ '.abq', '.dat', '.res','_RESTART.zip','.mdl','.sim','.odb','old.odb', '.stt', '.prt', ]              
    ...
    if excludePhrase == 0 or excludePhrase not in fname:
      ...
      if fname[-4:] in extensions:
        if len([each for each in os.listdir(root) if each.startswith(name[:-4])]) > 3:
          ...
          count += 1
          size += getsize(fname)
      elif fname[-4:] in timed_ext or '_RESTART.zip' in fname:
